...
top.by.var <- expression.data[,order(apply(expression.data, 2, var), decreasing = T)][,1:5000]

# Alternatively, keep 5,000 genes with highest coefficient of variation
CV <- function(vec){
  return(sd(vec)/mean(vec))
}
top.by.cv <- expression.data[,order(apply(expression.data, 2, CV), decreasing = T)][,1:5000]
...
pls.fit <- train(Response ~ .,          # formula format is response ~ predictors, see ?formula
                data = training,
                method = 'pls')
pls.fit

# Test on our testing data set
pls.preds <- predict(pls.fit, newdata = testing)
mse.pls <- MSE(yhat = pls.preds, y = testing[,'Response'])
